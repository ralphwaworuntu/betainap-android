        ...
        try {
            ...
            for (int i = 0; i < json_array.length(); i++) {
                ...
                try {
                    ...
                    if (json_event.has("cf_id") && !json_event.isNull("cf_id"))
                        event.setCf_id(json_event.getInt("cf_id"));

                    if (json_event.has("limit") && !json_event.isNull("limit"))
                        event.setLimit(json_event.getInt("limit"));

                    if (json_event.has("price") && !json_event.isNull("price"))
                        event.setPrice(json_event.getInt("price"));

                    if (json_event.has("priceFormatted") && !json_event.isNull("priceFormatted"))
                        event.setPriceFormatted(json_event.getString("priceFormatted"));

                    if (json_event.has("cf") && !json_event.isNull("cf")) {
                        EventCFParser mCFParser = new EventCFParser(new JSONObject(json_event.getString("cf")));
                        event.setCf0(mCFParser.getCFs());
                    }
