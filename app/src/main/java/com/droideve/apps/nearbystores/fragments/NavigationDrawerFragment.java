...
public class NavigationDrawerFragment extends Fragment implements SimpleListAdapterNavDrawer.ClickListener {
    ...
    public List<Item> getData() {
        ...
        chatItem.setName(getResources().getString(R.string.inbox));
        chatItem.setImageId(R.drawable.ic_chat_outline);
        chatItem.setID(Menu.CHAT);
        ...
        if (SettingsController.isModuleEnabled("event")) {
            Item myTicket = new Item();
            myTicket.setName(getResources().getString(R.string.my_ticket));
            myTicket.setIconDraw(CommunityMaterial.Icon2.cmd_ticket_outline);
            myTicket.setID(Menu.MY_TICKETS);
            listItems.add(myTicket);
        }
    ...
    public void itemClicked(View view, int position) {
        ...
        if (item instanceof Item) {
            ...
            switch (item.getID()) {
                ...
                case Menu.CHAT:
                    if (SessionsController.isLogged()) {
                        Intent messengerIntent = new Intent(getActivity(), InboxFragment.class);
                ...
                case Menu.MY_COUPONS:
                    ...
                    break;
                case Menu.MY_TICKETS:
                    if (SessionsController.isLogged()) {
                        startActivity(new Intent(getActivity(), MyTicketsListActivity.class));
                    }else{
                        startActivity(new Intent(getActivity(), LoginV2Activity.class));
                    }
                    break;
