...
public class BookingCheckoutActivity extends AppCompatActivity {
    ...
    private void handleIntentAction() {
        ...
        if (getIntent().hasExtra("module_id") && getIntent().hasExtra("module")) {
            ...
            if(Objects.equals(module, Constances.ModulesConfig.SERVICE_MODULE)){
                this.processForService();
            }else if(Objects.equals(module, Constances.ModulesConfig.EVENT_MODULE)){
    ...
    private void processForEvent(){
        ...
        if (getIntent().hasExtra("cart")) {
            ...
            bookingAmount = storedCart.getAmount() * storedCart.getQte();
            mCart = Arrays.asList(storedCart);
        }
    }
    ...
    private void generatePaymentLinkAPi(View v) {
        ...
        ApiRequest.newPostInstance(Constances.API.API_PAYMENT_LINK, new ApiRequestListeners() {
            ...
            public void onSuccess(Parser parser) {
                try {
                    displayPaymentWebView(parser.getJson().getString("result"));
                    //enable  button
                    ...
                    MessageDialog.showMessage(BookingCheckoutActivity.this,Map.of("err",e.getMessage()));
                }
            }

            @Override
            public void onFail(Map<String, String> errors) {
                MessageDialog.showMessage(BookingCheckoutActivity.this,errors);
            }
        }, params);
    ...
    private void showSuccessPage() {
        ...
        findViewById(R.id.lyt_done).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent bookingIntent = new Intent(BookingCheckoutActivity.this, BookingListActivity.class);
    ...
    private void submitOrderAPI() {
        ...
        if(mCart.get(0).getModule().equals(Constances.ModulesConfig.SERVICE_MODULE)){
            params.put("store_id", String.valueOf(module_id));
            params.put("module", String.valueOf(module));
            params.put("req_cf_id", String.valueOf(mStore.getCf_id()));
            params.put("booking_type", "service");
        }else  if(mCart.get(0).getModule().equals(Constances.ModulesConfig.EVENT_MODULE)){
            Event event = EventController.getEvent(module_id);
            params.put("store_id", String.valueOf(event.getStore_id()));
            params.put("event_id", String.valueOf(module_id));
            params.put("module", String.valueOf(module));
            params.put("req_cf_id", String.valueOf(mEvent.getCf_id()));
            params.put("booking_type", "digital");
        }
        ...
        try {
            ...
            for (Cart c : mCart) {
                ...
                if (c.getModule().equals(Constances.ModulesConfig.SERVICE_MODULE)
                && c.getServices() != null && c.getServices().size() > 0) {
                    ...
                    JSONObject cart = new JSONObject();
                    cart.put("qty", String.valueOf(c.getQte()));
                    cart.put("module_id", String.valueOf(c.getModule_id()));
                    cart.put("module", c.getModule());
                    cart.put("amount", String.valueOf(c.getAmount()));
                    carts.put(cart);
        ...
        ApiRequest.newPostInstance(Constances.API.API_BOOKING_CREATE, new ApiRequestListeners() {
            @Override
            public void onSuccess(Parser parser) {

                if (parser.getSuccess() == 1) {
                    try {
                        booking_id = parser.getJson().getInt("result");
                    } catch (JSONException e) {
                        throw new RuntimeException(e);
                    }
                    if (!SettingsController.isModuleEnabled(Constances.ModulesConfig.SERVICE_PAYMENT_MODULE)
                            || !SettingsController.isModuleEnabled(Constances.ModulesConfig.SERVICE_PAYMENT_MODULE) || bookingAmount == 0) {
                        showSuccessPage();
                    } else {
                    ...
                    if (orderFields != null && !orderFields.isEmpty()) {
                        ...
                        if(mCart.get(0).getModule().equals(Constances.ModulesConfig.SERVICE_MODULE)){
                            cfId = mStore.getCf_id();
                        }else if(mCart.get(0).getModule().equals(Constances.ModulesConfig.EVENT_MODULE)){
                            cfId = EventController.getEvent(mCart.get(0).getModule_id()).getCf_id();
                        }
