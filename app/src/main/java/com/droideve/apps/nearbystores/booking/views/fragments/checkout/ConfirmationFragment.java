...
public class ConfirmationFragment extends Fragment {
    ...
    private void parserInputViews(View view) {
        ...
        if(mCart.get(0).getModule().equals(Constances.ModulesConfig.SERVICE_MODULE)){
            //generate items for Store (Services)
    ...
    public void generateProductFromCartEvent(View view) {
        ...
        for (Cart c : mCart) {
            ...
            Event mEvent = EventController.getEvent(c.getModule_id());
            ...
            price.setId(R.id.desc_product);
            price.setTextColor(getResources().getColor(R.color.defaultColorText));
            LinearLayout.LayoutParams lp_match_wrap_c3 = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
            lp_match_wrap_c3.topMargin = (int) getResources().getDimension(R.dimen.spacing_medium);
            price.setLayoutParams(lp_match_wrap_c3);

            //dynamic content
            title_service.setText(mEvent.getName()+" x "+c.getQte());

            if( (c.getAmount()*c.getQte()) > 0 ){
                price.setText(String.valueOf(c.getAmount()*c.getQte()));
            }else{
                price.setText(mContext.getString(R.string.free));
            }

            sub_layout.addView(linearLayout_512);
            sub_layout.addView(price);
