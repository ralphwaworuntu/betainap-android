...
public class MessageDialog {
    ...
    public static void showMessageRegisterConfirmUser(Activity context, Map<String, String> messages, View.OnClickListener Oklistener){
        ...
        }).onOkClick(Oklistener).setContent(Translator.print(CommunFunctions.convertMessages(messages), "Message showError")).show();

        MessageDialog.getInstance().dialog.findViewById(R.id.cancel);

        MessageDialog.getInstance().dialog.setCancelable(true);

        TextView txt = MessageDialog.getInstance().dialog.findViewById(R.id.msgbox);
        txt.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);

        Button ok = MessageDialog.getInstance().dialog.findViewById(R.id.ok);
        ok.setText(context.getString(R.string.signUpAndVerify));
    }

    public static void showMessageRegisterUser(Activity context, Map<String, String> messages, View.OnClickListener Oklistener){
        ...
        MessageDialog.getInstance().dialog.findViewById(R.id.cancel).setVisibility(View.GONE);
       Button ok = MessageDialog.getInstance().dialog.findViewById(R.id.ok);
        ok.setText(context.getString(R.string.register));
    }

    public static void showMessage0(Activity context, Map<String, String> messages, View.OnClickListener Oklistener){
