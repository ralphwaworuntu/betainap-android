...
public class EditProfileFragment extends Fragment implements ImageUtils.PrepareImagesData.OnCompressListner {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.v2_fragment_profile_edit, container, false);
    ...
    private void loadProfileData() {
        ...
        if (mUser != null) {
            ...
            String NumberStr = mUser.getPhone();

            if(!NumberStr.equals("") && !NumberStr.equals("null")){
                ...
                try {
                    ...
                    phoneDialCode.setDefaultCountryUsingNameCode(regionISO);
                    phoneDialCode.resetToDefaultCountry();

                    phone.setText(String.valueOf(NumberProto.getNationalNumber()));

                } catch (NumberParseException e) {
                    phone.setText(mUser.getPhone().replace( "+"+phoneDialCode.getSelectedCountryCode().toString(),"" ));
                    return;
                }
            }
    ...
    private void doDisableAccount() {
        ...
        if (SessionsController.getSession() != null) {
            ...
            if (mUser != null) {
                ...
                ApiRequest.newPostInstance(Constances.API.API_DISABLE_ACCOUNT, new ApiRequestListeners() {
                    @Override
                    ...
                    public void onFail(Map<String, String> errors) {
                        ...
                        "user_token", SessionsController.getSession().getToken()
                ));
            }
    ...
    private void doSave() {
        ...
        //Veify Phone Number OTP
        if(!phoneIsVerified){

            if(phone.getText().toString().trim().equals("")){
                MessageDialog.showMessage(getActivity( ), Map.of("err",getString(R.string.phone_empty)));
                return;
            }

            String newPhone  = "+"+phoneDialCode.getSelectedCountryCode().toString() + phone.getText().toString().trim();
            Setting otpEnabled = SettingsController.findSettingFiled("OTP_ENABLED");
            if( otpEnabled!=null &&  Integer.parseInt(otpEnabled.getValue()) == 1 && !newPhone.equals(mUser.getPhone())){
                MessageDialog.showMessageRegisterConfirmUser(getActivity(), Map.of("err",getString(R.string.verify_phone_number_otp)), new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        i.putExtra("phoneNumber",phone.getText().toString().trim().trim());
        ...
        params.put("phone", "+"+phoneDialCode.getSelectedCountryCode().toString().trim()+phone.getText().toString().trim().trim());
        ...
        ApiRequest.newPostInstance(Constances.API.API_UPDATE_ACCOUNT, new ApiRequestListeners() {
            @Override
            public void onSuccess(Parser parser) {
                ...
                if (mUserParser.getSuccess() == 1) {
                    ...
                    if (list.size() > 0) {
                        ...
                        String oldPhone = mUser.getPhone();

                        //Show message
                        NSToast.show(getString(R.string.updateSuccessful));
                        SessionsController.logOut();
