...
public class EventListAdapter extends RecyclerView.Adapter<EventListAdapter.mViewHolder> {
    ...
    public EventListAdapter.mViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        else rootView = infalter.inflate(R.layout.v3_item_event, parent, false);
    ...
    public void onBindViewHolder(EventListAdapter.mViewHolder holder, int position) {
        ...
        if (this.data.get(position).getListImages() != null
                && this.data.get(position).getListImages().size()> 0
                && this.data.get(position).getListImages().get(0).getUrl200_200() != null
        ) {
        ...
        if(data.get(position).getPrice()==0){
            holder.price.setText(context.getString(R.string.event_free));
            holder.price.setVisibility(View.VISIBLE);
        }else {
            holder.price.setText(data.get(position).getPriceFormatted());
            holder.price.setVisibility(View.VISIBLE);
        }
    }
